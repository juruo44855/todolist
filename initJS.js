var myAllTasks = new Array(); //store  tasks
const month = {
  1: "Jan",
  2: "Feb",
  3: "Mar",
...
$(document).ready(function () {
  ...
  $("#linearButton").on("click", function () {
    if (
      $("#linearBackground").is(":visible") ||
      $("#linearBackground").is(":hidden")
    ) {
      $("#linearBackground").hide();
    } else {
      $("#linearBackground").show();
    }
  });
  ...
  $("#addButton").on("click", function () {
    ...
    /*
        <ul>
          ...
          <div class="functionField">
            <button class="editIMG">
              <i class="fas fa-pencil-alt me-3" title="Edit todo"></i>
            </button>
            <button class="delIMG">
    ...
    */
    ...
    let dolistobj = { dueDate: settingTime, inputField: $("#inputField").val() }
    $.get("/main/dolist", dolistobj, function (res) {
      alert("Saving dolist!");
      console.log(res);
    });
...
function loaddb() {
  console.log("loaddb");
  $.get("/main/loaddolist", function (res) {
    console.log(res);
    res=JSON.parse(res);
    for(re in res){
      loaddolist(res[re].inputField,res[re].dueDate)
    }
...
function loaddolist(field, time) {
  //add task
    //check if input field is empty , if yes =>show the warning
    if (field == "") {
      $("#popOut").stop(true).fadeIn(800);
